def generate_markdown(analyzer) -> str:
    ...
    report += f"- 数据的来源 {analyzer.filepath}\n\n"
    report += f"##各列概况\n\n"
    report += f"烈数|总数|缺失值|缺失率|样例|\n"
    report += f"|------||------||------||------||------|\n"
    for col, info in summary().items():
        sample_str = ','.join(str(v) for v in info['sample'][:2])
        report += f"|{col}|{info['total']}|{info['missing']}|{info['missing_pct']}|{info[sample_str]}\n"
